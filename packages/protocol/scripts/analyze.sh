...
#
# Usage: ./scripts/analyze.sh [circuit|ecne|slither|aderyn|contracts|all]
set -uo pipefail
...
case "${1:-all}" in
  ...
  all)       run_circuit; run_ecne; run_slither; run_aderyn ;;
  *)         echo "usage: $0 [circuit|ecne|slither|aderyn|contracts|all]"; exit 2 ;;
esac
